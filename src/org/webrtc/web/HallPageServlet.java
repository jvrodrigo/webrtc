...
/**
 * @author jvrodrigo
 * @apiNote Welcome class to store the user name and a token
 */
public class HallPageServlet extends HttpServlet {
	/**
	 * 
	 */

	private static final long serialVersionUID = 1L;
	private static final Logger logger = Logger.getLogger(HallPageServlet.class.getName());
	...
	public void doPost(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		for (Entry<String, User> user : userList.entrySet()) {
			System.out.println(
					"Usuarios conectados: Nombre -> " + user.getValue().getName() + " | Token -> " + user.getValue().getToken());
			if (!this.user.getToken().equals(user.getValue().getToken())) {
				userListLi = userListLi
						+ "<li title=\"Pulse para llamar al usuario\" class=\"users-list-li\" onclick=\"calling(this)\" " + "id=\""
						+ user.getValue().getToken() + "\"><p>" +
						// "<a href=\"/webrtc/?r=" + user.getValue().getToken() + this.user.getToken() +
						// "\">Usuario:" +
						// user.getValue().getName() +
						// "</a>" +
						"Usuario: " + user.getValue().getName() + "</p></li>";
			}
		...
		logger.info("Nuevo usuario conectado: " + user.getName() + " token " + user.getToken());
	...
	public static void setUserList(ConcurrentMap<String, User> userList) {
		HallPageServlet.userList = userList;
	}
}